...
**会话重用 (Session Reuse)**:
An optional, off-by-default setting (`reuseSession`, the 「重用会话」 checkbox on the API tab) that gives each 并发池 worker one conversation chain. A slot's earlier _successful_ segments are replayed to the model as the conversation history — each segment's original skeleton HTML as the user turn, the model's raw echoed reply as the assistant turn — so the model reads the terminology and style it established on that slot and translates only the newest text, which arrives bare: the new segment alone, with no prompt template (the template renders into the chain's first request only, and each replayed user turn is wrapped in exactly one `<html>…</html>` wrapper by the same helper that injects `{host}`, `prompt.js` being the single source of the wrapper form). The history is plain `messages` accumulated client-side — no server-side session id, nothing stored outside the content script: the chain belongs to the slot, not to the page or to document order (N in-flight hosts, N independent chains; concurrency semantics unchanged), and it is created and dropped with the 并发池 of one round of translation, so 还原 or a page refresh discards it and turning the option on or off takes effect on the next round, never inside a chain already in flight. A segment that fails — endpoint error, connection loss, empty echo — never enters the chain, so a chain is exactly its slot's successful segments in order. Two failure modes drop the whole history and retry that segment once as a new session (chain head, no history): the echoed reply is not exactly one `<html>…</html>` wrapper — this trigger applies whether or not the option is on, and it replaces the old lenient rendering of an off-shape echo — or the endpoint reports a context-limit error (this one only when the request actually carried history — a chain head is the stateless request, so resending it would change nothing). Both retries happen inside background: content still sees exactly one `done` or one `error` per host and never an extra settle, so 进度徽标 counting is unaffected. A retry that succeeds makes that segment the slot's new chain head (the dropped history is not carried forward); a retry that fails too takes the ordinary error path. Empty echoes (`""` / `<html></html>`) are never retried — they keep the 「未返回译文」 error.
_Avoid_: server session, session id, chat memory
...
**进度徽标 (Progress Badge)**:
A viewport-fixed overlay at the bottom-right corner showing live session progress: `<pct>% <done>/<total>(<totalAll>)(<errors>)`. The fraction counts only hosts outside 边缘区域 (tiers 0/1), so the percentage tracks main-content progress and can never exceed 100%; `<totalAll>` counts every discovered host including 边缘区域; the `<errors>` bracket is shown only while at least one host request (any tier) has failed. The percentage floors. The badge appears on Translate, stays at 100% as the completion signal, and 还原 removes it. Once every discovered host has settled, the badge gains a trailing timing reading, making the full format `<pct>% <done>/<total>(<totalAll>)(<errors>) <main>(<all>)s`: `<main>` is the seconds elapsed at the moment the progress fraction's hosts (tiers 0/1) all settled, `<all>` the seconds elapsed at the moment every discovered host, 边缘区域 included, all settled; both floor like the percentage and are taken once, at their own settling moment — fixed readings of a finished pass, not a running clock. The reading exists only while everything has settled: a rescan that discovers new hosts takes it down, and it returns, still accumulating from Translate, once those hosts settle — only a new main-content host unfreezes both numbers, and new 边缘区域 hosts alone renew `<all>` while `<main>` keeps its earlier reading. A page with no discovered host at all never shows a timing reading, and a page with no main-content host reads `0` outside the bracket. 还原 clears it with the badge. As an injected element it never becomes a 宿主 and never triggers a rescan.
