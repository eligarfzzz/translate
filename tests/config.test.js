...
// 加载顺序约定：lib/prompt.js 先于 config.js（config 的 mergeConfig 依赖其全局常量）
require("../lib/prompt.js");
const { mergeConfig, mergeConfigRaw, loadConfig, loadConfigRaw, TRANSLATE_CONFIG } = require("../config.js");
...
function DEFAULT_PROMPT_TEMPLATE() {
  // lib/prompt.js 挂载的全局常量（require 时已就位）
  return globalThis.DEFAULT_PROMPT_TEMPLATE ?? require("../lib/prompt.js").DEFAULT_PROMPT_TEMPLATE;
}
...
test("mergeConfig 判空：null/undefined/空串回退默认，非空字符串（含纯空白）原样透传", () => {
  const cfg = mergeConfig(TRANSLATE_CONFIG, { model: "", targetLang: "   ", apiBase: "https://example.com/v1" });
...
test("mergeConfig 透传非空 promptTemplate（含不含 {host} 的），空串/缺失回退默认", () => {
  ...
  // 空串/缺失 → 默认模板
  assert.equal(mergeConfig(TRANSLATE_CONFIG, { promptTemplate: "" }).promptTemplate, DEFAULT_PROMPT_TEMPLATE());
  assert.equal(mergeConfig(TRANSLATE_CONFIG, { promptTemplate: null }).promptTemplate, DEFAULT_PROMPT_TEMPLATE());
...
test("mergeConfigRaw UI 语义：保存什么读什么，空值透传为空串", () => {
  ...
  const filled = mergeConfigRaw(TRANSLATE_CONFIG, { model: "m1", promptTemplate: "no host", targetLang: "" });
...
test("loadConfig 在 node 环境（无 chrome）返回默认合并", async () => {
  if (typeof globalThis.chrome !== "undefined") return; // 浏览器化环境跳过
...
test("loadConfig 经 chrome.storage.sync 合并存储覆盖", async () => {
  ...
  globalThis.chrome = {
    storage: {
      sync: {
        get: async (key) => (key === "config" ? { config: { model: "stored-model", concurrency: -1 } } : {}),
      },
    },
  };
  try {
    ...
    if (saved === undefined) delete globalThis.chrome;
    else globalThis.chrome = saved;
  }
});
